  ...
  async fetch(request, env) {
      ...
      if (访问路径 === 加密秘钥 && 加密秘钥 !== "勿动此默认密钥，有需求请自行通过添加变量KEY进行修改") {
        ...
        if (request.method === "POST") {
          ...
          if (输入密码 === 管理员密码) {
            ...
            响应.headers.set(
              "Set-Cookie",
              `auth=${await MD5MD5(UA + 加密秘钥 + 管理员密码)}; Path=/; Max-Age=86400; HttpOnly`
            );
            return 响应;
          }
        }
        ...
        if (url.searchParams.get("token") === 订阅TOKEN) {
          ...
          if (订阅类型 === "mixed") {
            ...
            if (!url.searchParams.has("sub") && config_JSON.优选订阅生成.local) {
              ...
              订阅内容 = 完整优选IP
                .map((原始地址) => {
                  ...
                  return `${协议类型}://${config_JSON.UUID}@${节点地址}:${节点端口}?security=tls&type=${
                    config_JSON.传输协议
                  }&host=${节点HOST}&sni=${节点HOST}&path=${
                    encodeURIComponent(config_JSON.随机路径 ? 随机路径() + 节点路径 : 节点路径) + TLS分片参数
                  }&encryption=none${config_JSON.跳过证书验证 ? "&allowInsecure=1" : ""}#${encodeURIComponent(
                    节点备注
                  )}`;
                })
            ...
            // 订阅转换
            const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(
              url.protocol +
                "//" +
                url.host +
                ...
                (url.searchParams.has("sub") && url.searchParams.get("sub") != ""
                  ...
                  : "")
            )}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${
              config_JSON.订阅转换配置.SUBEMOJI
            }&scv=${config_JSON.跳过证书验证}`;
            try {
              const response = await fetch(订阅转换URL, {
                headers: {
                  "User-Agent":
                    "Subconverter for " + 订阅类型 + " edge" + "tunnel(https://github.com/cmliu/edge" + "tunnel)",
                },
              });
              if (response.ok) {
          ...
          if (订阅类型 === "mixed") {
            订阅内容 = 批量替换域名(
              atob(订阅内容).replace(/00000000-0000-4000-8000-000000000000/g, config_JSON.UUID),
              host
            );
    ...
    try {
      ...
      if (新请求头.has("Referer")) {
        ...
        新请求头.set("Referer", 反代URL.protocol + "//" + 反代URL.host + u.pathname + u.search);
      }
      if (新请求头.has("Origin")) 新请求头.set("Origin", 反代URL.protocol + "//" + 反代URL.host);
      if (!新请求头.has("User-Agent") && UA && UA !== "null") 新请求头.set("User-Agent", UA);
      return fetch(
        new Request(反代URL.protocol + 反代URL.host + url.pathname + url.search, {
...
async function 读取config_JSON(env, hostname, userID, 重置配置 = false) {
  ...
  const TLS分片参数 =
    config_JSON.TLS分片 == "Shadowrocket"
      ...
      ? `&fragment=${encodeURIComponent("3,1,tlshello")}`
      : "";
  config_JSON.LINK = `${config_JSON.协议类型}://${userID}@${host}:443?security=tls&type=${
    config_JSON.传输协议
  }&host=${host}&sni=${host}&path=${
    encodeURIComponent(config_JSON.启用0RTT ? config_JSON.PATH + "?ed=2560" : config_JSON.PATH) + TLS分片参数
  }&encryption=none${config_JSON.跳过证书验证 ? "&allowInsecure=1" : ""}#${encodeURIComponent(
    config_JSON.优选订阅生成.SUBNAME
  )}`;
        ...
        if (!isCSV) {
          lines.forEach((line) => {
            ...
            const [hostPart, remark] =
              hashIndex > -1 ? [line.substring(0, hashIndex), line.substring(hashIndex)] : [line, ""];
            let hasPort = false;
...
async function 反代参数获取(request) {
  ...
  if ((socksMatch = pathname.match(/\/(socks5?|http):\/?\/?(.+)/i))) {
    ...
    if (我的SOCKS5账号.includes("@")) {
      ...
      if (
        /^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i.test(userPassword) &&
...
async function 获取SOCKS5账号(address) {
  ...
  const [hostPart, authPart] =
    atIndex === -1 ? [address, undefined] : [address.substring(atIndex + 1), address.substring(0, atIndex)];
...
function sha224(s) {
  ...
  for (let i = 0; i < w.length; i += 16) {
    ...
    for (let j = 0; j < 8; j++)
      h[j] =
        (h[j] +
          (j === 0 ? a : j === 1 ? b : j === 2 ? c : j === 3 ? d : j === 4 ? e : j === 5 ? f : j === 6 ? g : h0)) >>>
        0;
  }
  let hex = "";
  for (let i = 0; i < 7; i++) {
...
async function SOCKS5可用性验证(代理协议 = "socks5", 代理参数) {
  ...
  try {
    ...
    try {
      ...
      await writer.write(
        encoder.encode(`GET /cdn-cgi/trace HTTP/1.1\r\nHost: check.socks5.090227.xyz\r\nConnection: close\r\n\r\n`)
      );
      writer.releaseLock();
